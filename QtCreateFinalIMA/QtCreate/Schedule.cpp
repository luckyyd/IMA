...
void QtCreate::PartitionScheduler(int partitionnum)
{
	int timeslice = 0;
	bool flag = false;
	bool flagInDeparture = false;
	bool flagOutDeparture = false;
	...
	while (timeslice < MAXTIMESLICE)
	{
		...
		for (int i = 1; i <= partitionnum; i++)
		{
			if (p[i].offset <= timeslice && p[i].initonce == false)
			{
				...
				//Departure & Arrival
				if (timeslice >= 25 && timeslice <= 100 && flagInDeparture == false)
				{
					for (int i = 1; i <= partitionnum; i++)
					{
						if (p[i].intensiveperiod == 2)
						{
							p[i].period = p[i].period / 6;
							p[i].duration = p[i].duration / 4;
							if (p[i].period <= p[i].duration)
							{
								p[i].duration == p[i].period;
							}
							flagInDeparture = true;
						} 
					}
				}
				if (timeslice > 100 && flagOutDeparture == false)
				{
					for (int i = 1; i <= partitionnum; i++)
					{
						if (p[i].intensiveperiod == 2)
...
void QtCreate::TaskScheduler(int timeslice, int tasknum, int partition)
{
	...
	while (timeslice < slice + p[partition].duration)
	{
		if (timeslice == 35)
		{
			ui.ConsoleTextEdit->append(QString("GGG"));
		}
